...
exports.mailValidationZeroBounce = async (req, res, next) => {
  try {
    const checkEmail = await axios.get(`https://api.zerobounce.in/v2/validate?api_key=eeaa29b0c77a4cba96e66e61698d7b0f&email=${req.body.email}`);
    console.log("checkEmail.data", checkEmail.data);
    if (checkEmail && checkEmail.data) {
      if ((checkEmail.data && checkEmail.data.status === "valid") || (checkEmail.data && checkEmail.data.status === "catch-all")) {
        return next();
      } else {
        return res.status(400).json({
          ...
          status: 400,
          error: true,
          message: "Email address invalid or not correct domain provided in email address.",
        });
      }
    } else {
      return next();
    ...
    console.log("Error In Email Validation Middleware", error.message);
    return next();
  }
}